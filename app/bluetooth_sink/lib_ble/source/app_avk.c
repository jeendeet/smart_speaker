...
#define PAYLOAD_TO_AVS_MAX                  100
#define MQTT_AVS_TOPIC                      "bluetooth/avs-audio-control"

#define APP_MGR_KEY_CALLBACK                "bsa"
#define APP_MGR_CALLBACK_PAIR_STATUS        "PAIR-STATUS"
#define APP_MGR_CALLBACK_STREAM_PLAY        "STREAM-PLAY"
#define APP_MGR_CALLBACK_STREAM_STOP        "STREAM-STOP"

#define APP_MGR_KEY_STATE                   "state"
#define APP_MGR_MENU_PAIR_SUCCESS           "PAIR-SUCCESS"
#define APP_MGR_MENU_PAIR_FAIL              "PAIR-FAIL"

#define APP_MGR_KEY_MAC_ADDRESS             "addr"
...
tAPP_AVK_CONNECTION *last_connection = NULL;
...
static void app_avk_cback(tBSA_AVK_EVT event, tBSA_AVK_MSG *p_data)
{
    ...
    case BSA_AVK_START_EVT:
        ...
        else
        {
            ...
            app_update_json_item("av_state", "PLAY");
            if (app_audio_get_bluetooth_focus_avs() == FALSE)
            {
                char payload[PAYLOAD_TO_AVS_MAX]; strcpy((char*)payload, "");
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_STREAM_PLAY);
                app_audio_client_mqtt(MQTT_AVS_TOPIC, (char*)payload);
                APP_DEBUG0("BBBBBBBBBBBBBBBBBBBBBBBBBBB BSA_AVK_START_EVT 1 BBBBBBBBBBBBBBBBBBBBBBBBBBBB");
            }
            else
            {
                app_audio_set_bluetooth_focus_avs(FALSE);
                APP_DEBUG0("BBBBBBBBBBBBBBBBBBBBBBBBBBB BSA_AVK_START_EVT 2 BBBBBBBBBBBBBBBBBBBBBBBBBBBB");
            }
        }
    ...
    case BSA_AVK_STOP_EVT:
        ...
        app_update_json_item("av_state", "STOP");
        if (app_audio_get_bluetooth_focus_avs() == FALSE)
        {
            char payload[PAYLOAD_TO_AVS_MAX]; strcpy((char*)payload, "");
            app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_STREAM_STOP);
            app_audio_client_mqtt(MQTT_AVS_TOPIC, (char*)payload);
            APP_DEBUG0("BBBBBBBBBBBBBBBBBBBBBBBBBBB BSA_AVK_STOP_EVT 1 BBBBBBBBBBBBBBBBBBBBBBBBBBBB");
        }
        else
        {
            app_audio_set_bluetooth_focus_avs(FALSE);
            APP_DEBUG0("BBBBBBBBBBBBBBBBBBBBBBBBBBB BSA_AVK_STOP_EVT 2 BBBBBBBBBBBBBBBBBBBBBBBBBBBB");
        }
    ...
    case BSA_AVK_REMOTE_CMD_EVT:
        ...
        APP_DUMP((char *)"data", p_data->remote_cmd.data, p_data->remote_cmd.len);
    ...
    case BSA_AVK_SET_ABS_VOL_CMD_EVT:
        ...
        {
            APP_DEBUG1("BSA_AVK_SET_ABS_VOL_CMD_EVT %d", app_avk_cb.volume);
...
void app_avk_open(void)
{
    ...
    case 0:
        ...
        //     app_xml_remote_devices_db[1].bd_addr[0], app_xml_remote_devices_db[1].bd_addr[1],
        //     app_xml_remote_devices_db[1].bd_addr[2], app_xml_remote_devices_db[1].bd_addr[3],
        //     app_xml_remote_devices_db[1].bd_addr[4], app_xml_remote_devices_db[1].bd_addr[5]);
        // APP_DEBUG1("macStr BBBBBBBBBBBBBBBBBBBBBBBBBBB %s", macStr);
        // // choice = app_get_choice("Select device");
        // if (APP_NUM_ELEMENTS(app_xml_remote_devices_db) != 0) {
        //     for (int i = 0; i < APP_NUM_ELEMENTS(app_xml_remote_devices_db); i++)
        //     {
        //         snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
        //             app_xml_remote_devices_db[i].bd_addr[0], app_xml_remote_devices_db[i].bd_addr[1],
        //             app_xml_remote_devices_db[i].bd_addr[2], app_xml_remote_devices_db[i].bd_addr[3],
        //             app_xml_remote_devices_db[i].bd_addr[4], app_xml_remote_devices_db[i].bd_addr[5]);
        //         APP_DEBUG1("macStr %s", macStr);
        //         APP_DEBUG1("bdaddr %s", app_get_json_item("bdaddr"));
        //         APP_DEBUG1("i %d", i);
        //         if (strcmp((char *)macStr, app_get_json_item("bdaddr")) == 0)
        //         {
        //             choice = i;
        //             APP_DEBUG0("macStr AAAAAAAAAAAAAAAAAAAAA");
        //         }
        //     }
        // }
        APP_DEBUG1("Choice %d", choice);
        if ((choice >= 0) && (choice < APP_NUM_ELEMENTS(app_xml_remote_devices_db)))
        {
            if (app_xml_remote_devices_db[choice].in_use != FALSE)
            {
                bdcpy(bd_addr, last_connection->bda_connected);
    ...
    if (connect != FALSE)
    {
        ...
        snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
                open_param.bd_addr[0], open_param.bd_addr[1],
                open_param.bd_addr[2], open_param.bd_addr[3],
                open_param.bd_addr[4], open_param.bd_addr[5]);
        ...
        else
        {
            ...
            if(connection == NULL || connection->is_open == FALSE)
            {
                ...
                char payload[PAYLOAD_TO_AVS_MAX]; strcpy((char*)payload, "");
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_PAIR_STATUS);
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_STATE, (char*)APP_MGR_MENU_PAIR_FAIL);
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_MAC_ADDRESS, (char*)macStr);
                
                app_update_json_item("state", "UNPAIR");
            ...
            else
            {
                ...
                char payload[PAYLOAD_TO_AVS_MAX]; strcpy((char*)payload, "");
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_PAIR_STATUS);
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_STATE, (char*)APP_MGR_MENU_PAIR_SUCCESS);
                app_audio_create_json_str(payload, (char*)APP_MGR_KEY_MAC_ADDRESS, (char*)macStr);

                app_update_json_item("state", "PAIR");
