...
/* Default local Name */
#define APP_DEFAULT_BT_NAME                 "VBDSpeaker"
...
#define APP_MGR_CALLBACK_LINK_UP            "LINK_UP"
#define APP_MGR_CALLBACK_LINK_DOWN          "LINK_DOWN"
#define APP_MGR_CALLBACK_AUTH               "AUTHORIZE"

#define APP_MGR_KEY_STATE                   "state"
#define APP_MGR_MENU_BLUE_CONNECT           "BLUE-CONNECT"
#define APP_MGR_MENU_BLUE_DISCONNECT        "BLUE-DISCONNECT"

#define APP_MGR_KEY_FRIENDLY_NAME           "name"
...
void app_mgr_security_callback(tBSA_SEC_EVT event, tBSA_SEC_MSG *p_data)
{
    ...
    case BSA_SEC_LINK_UP_EVT:       /* A device is physically connected (for info) */
        ...
        if (APP_NUM_ELEMENTS(app_xml_remote_devices_db) != 0) {
            ...
            {
                snprintf(macDBStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
                ...
                if (strcmp((char *)macDBStr, (char *)macStr) == 0)
                {
                    app_update_json_item("name", (char*)(app_xml_remote_devices_db[i].name));
                }
            }
        }

        app_update_json_item("state", "PAIR");
        app_update_json_item("bdaddr", (char*)macStr);

        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_LINK_UP);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_STATE, (char*)APP_MGR_MENU_BLUE_CONNECT);
    ...
    case BSA_SEC_LINK_DOWN_EVT:     /* A device is physically disconnected (for info)*/
        ...
        snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
                ...
                p_data->link_down.bd_addr[2], p_data->link_down.bd_addr[3],
                p_data->link_down.bd_addr[4], p_data->link_down.bd_addr[5]);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_LINK_DOWN);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_STATE, (char*)APP_MGR_MENU_BLUE_DISCONNECT);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_MAC_ADDRESS, (char*)macStr);
    ...
    case BSA_SEC_AUTH_CMPL_EVT:
        ...
        // system("LD_LIBRARY_PATH=/opt/sysroot/system_blue_lib /opt/app_avk &");
        break;
    ...
    case BSA_SEC_AUTHORIZE_EVT:  /* Authorization request */
        ...
        snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
                ...
                p_data->authorize.bd_addr[2], p_data->authorize.bd_addr[3],
                p_data->authorize.bd_addr[4],p_data->authorize.bd_addr[5]);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_CALLBACK, (char*)APP_MGR_CALLBACK_AUTH);
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_FRIENDLY_NAME, (char*)(p_data->authorize.bd_name));
        app_audio_create_json_str(payload, (char*)APP_MGR_KEY_MAC_ADDRESS, (char*)macStr);
        
        app_update_json_item("state", "PAIR");
        app_update_json_item("bdaddr", (char*)macStr);
        app_update_json_item("name", (char*)(p_data->authorize.bd_name));
